...
function isCardioWorkoutExercise(ex: any) {
  return (
    ex?.exercises?.exercise_type === 'cardio' ||
    ex?.exercises?.muscle_group === 'Cardio'
  );
}
  ...
  const addExerciseToSession = async () => {
    ...
    try {
      ...
      const nextOrder = exercises.length;
	      ...
	      // Pull exercise metadata so we can branch: strength is set-based, cardio is time-based.
				    ...
				    <div className="mt-3 flex flex-col sm:flex-row sm:items-end gap-3">
				      <div className="flex-1">
				        ...
				        <input
				          ...
				          inputMode="numeric"
				          placeholder="0:00"
				          value={cardioDraft[exercise.id] ?? formatMMSSFromSeconds(exercise.duration_seconds)}
				          onChange={(e) =>
				      ...
				      <button
				        ...
				        disabled={sessionIsCompleted}
				        onClick={async () => {
				          const secs = parseMMSS(cardioDraft[exercise.id] ?? formatMMSSFromSeconds(exercise.duration_seconds));
				          if (!secs || secs <= 0) {
